...
void SetupInterrupts()
{
	...
	Table->SVC = ARM_OPCODE_BRANCH(ARM_DISTANCE(Table->SVC, SVCStub));
	...
	Write32(ARM_IC_DISABLE_IRQS_1, (uint32) -1);
	Write32(ARM_IC_DISABLE_IRQS_2, (uint32) -1);
	Write32(ARM_IC_DISABLE_BASIC_IRQS, (uint32) -1);
...
void InterruptHandler(void)
{
	...
	for (unsigned IRQ = 0; IRQ < IRQ_LINES; IRQ++)
	{
		uint32 PendReg = ARM_IC_IRQ_PENDING(IRQ);
		uint32 IRQMask = ARM_IRQ_MASK(IRQ);
