...
static volatile bool WasCriticalEnabled;
...
void InvalidateDataCache (void)
{
	...
	for (register unsigned nSet = 0; nSet < L1_DATA_CACHE_SETS; nSet++)
	{
		...
		{
			register uint32 nSetWayLevel =   nWay << L1_SETWAY_WAY_SHIFT
	...
	for (register unsigned nSet = 0; nSet < L2_CACHE_SETS; nSet++)
	{
		...
		{
			register uint32 nSetWayLevel =   nWay << L2_SETWAY_WAY_SHIFT
...
void InvalidateDataCacheRange (uint32 nAddress, uint32 nLength)
{
...
void CleanDataCacheRange (uint32 nAddress, uint32 nLength)
{
	nLength += DATA_CACHE_LINE_LENGTH_MIN;
...
void CleanAndInvalidateDataCacheRange (uint32 nAddress, uint32 nLength)
{
	nLength += DATA_CACHE_LINE_LENGTH_MIN;
